...
% if isfield(net, 'visualize') && net.visualize > 0
%   opts.stopLayer = net.visualize;
% end
...
for l = obj.executionOrder
  ...
  if ~isempty(opts.stopLayer) && opts.stopLayer == l
    break;
  end
...
for i = 1:2:numel(derOutputs)
  v = obj.getVarIndex(derOutputs{i}) ;
  if ~isnan(v)
      switch obj.device
        case 'cpu', obj.vars(v).der = gather(derOutputs{i+1}) ;
        case 'gpu', obj.vars(v).der = gpuArray(derOutputs{i+1}) ;
      end
  end
end

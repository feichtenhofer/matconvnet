      ...
      for (int l = 0; l < numLevels ; l++) {
        ...
        for (int ph = 0; ph < pooledHeight; ++ph) {
          for (int pw = 0; pw < pooledWidth; ++pw) {
            ...
            int hstart = static_cast<int>(floor(static_cast<type>(ph)
                                                * bin_size_h));
            int wstart = static_cast<int>(floor(static_cast<type>(pw)
                                                * bin_size_w));
            int hend = static_cast<int>(ceil(static_cast<type>(ph + 1)
                                             * bin_size_h));
            int wend = static_cast<int>(ceil(static_cast<type>(pw + 1)
                                             * bin_size_w));
  ...
  for (size_t n = 0; n < numROIs; ++n) {
    ...
    for (int z = 0; z < depth; ++z) {
      for (int l = 0; l < numLevels ; l++) {
        ...
        type const bin_size_h = static_cast<type>(roi_height)
            / static_cast<type>(pooledHeight);
        type const bin_size_w = static_cast<type>(roi_width)
            / static_cast<type>(pooledWidth);
...
namespace vl { namespace impl {
    ...
    template <typename type>
    struct spp_average<vl::VLDT_CPU, type>
    {
      static vl::ErrorCode
      forward(type* pooled,
              type const* data,
              ...
              size_t numROIs, type const* ROIs)
      {
        ...
        spp_forward_cpu<type, acc_sum<type> > (pooled,
                                               data,
      ...
      backward(type* derData,
               ...
               size_t numROIs, type const* ROIs)
      {
        spp_backward_cpu<type, acc_sum<type> > (derData, data, derPooled,
                                                height, width, depth, size,
                                                numLevels, levels,
                                                numROIs, ROIs );
        return VLE_Success ;
    ...
    template <typename type>
    struct spp_max<vl::VLDT_CPU, type>
    {
      static vl::ErrorCode
      forward(type* pooled,
              type const* data,
              ...
              size_t numROIs, type const * ROIs)
      {
        ...
        spp_forward_cpu<type, acc_max<type> > (pooled,
                                               data,
                                               height, width, depth, size,
                                               numTotBins,
                                               numLevels, levels,
      ...
      backward(type* derData,
               ...
               size_t numROIs,  type const* ROIs)
      {
        spp_backward_cpu<type, acc_max<type> > (derData, data, derPooled,
                                                height, width, depth, size,
                                                numLevels, levels,
                                                numROIs, ROIs );
        return VLE_Success ;
...
// Instantiations
template struct vl::impl::spp_average<vl::VLDT_CPU, float> ;
template struct vl::impl::spp_max<vl::VLDT_CPU, float>;

#ifdef ENABLE_DOUBLE
template struct vl::impl::spp_average<vl::VLDT_CPU, double> ;
template struct vl::impl::spp_max<vl::VLDT_CPU, double> ;
#endif

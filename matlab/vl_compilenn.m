...
arch_code = gpu_device.ComputeCapability;
if ispc || str2num(arch_code) > 5
  arch_code = [];
else
  arch_code = strrep(arch_code, '.', '');
end
mex_dir = fullfile(root, 'matlab', ['mex' arch_code]) ;
...
if status, fprintf('Command %s failed.\n', nvcc_cmd); keyboard; end;
